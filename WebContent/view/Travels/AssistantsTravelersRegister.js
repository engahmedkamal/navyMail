...
function print_travelers_report_units(){
	...
	superConnect(api_source + 'ReportUnits/Print', report_params, function(obj) {
		var win = window.open('http://'+host_name+':3000/SubOfficerReport.pdf', '_blank');
		win.focus();
	});
}
...
function Push_Passengers_Array() {
	...
	var passenger_unit = "<select id='traveler_unit' class='form-control'>" + passengers_units + "</select>";
	// var passenger_position = "<input id='traveler_position' class='form-control'>";
	var passenger_position = "<select id='traveler_position' class='form-control'>" + passenger_position_options + "</select>";
	var notes = "<input class='form-control' id='traveler_add_notes' >";
	// var security_status = "<input id='traveler_security_status' type='checkbox'>";
	var security_status = "<select id='traveler_security_status' class='form-control'>" + passenger_security_options + "</select>";


	Passengers_Number += 1 ; 
	
	Passengers_Array.push([ , ,military_mumber,rank_select,
				passenger_name, ,passenger_unit,
				passenger_position,security_status,notes ]);
...
function ViewPassengersTable() {
	...
	for ( var row = 0; row < Passengers_Number; row++) {
		...
		var unit_name_tag = (row != Passengers_Number-1) ? internal_units_name[Passengers_Array[row][6]] : Passengers_Array[row][6] ; 
		var sec_status = Passengers_Array[row][8]  ; 
		if ((row != Passengers_Number-1)){ 
...
//				+notes
	...
	$("#m_num").bind('keypress',function(e){
		...
		if(e.keyCode == 13){
				...
				var unitid = $("#traveler_unit").find("option:selected").attr('value');
				if (rankid == -1 || unitid == -1){
					alert("تأكد من إدخال جميع البيانات"); 
					return ;
				}
				military_num_array.push($("#m_num").val()) ;
/*				var sec_status  = $("#traveler_security_status").val(); 
				...
				var traveler_sec_status  = $("#traveler_security_status").find("option:selected").attr('value');
				traveler_position =  $("#traveler_position").find("option:selected").attr('value'); 
				Passengers_Array[Passengers_Number-1]=[,,$("#m_num").val(),rankid,
				                                       $("#traveler_name").val(), ,
				                                       unitid,traveler_position,
				                                       traveler_sec_status,$("#traveler_add_notes").val()];
...
/*				var sec_status  = $("#traveler_security_status").val(); 
	...
	function Search_Traveler_By_Id()
	{
		...
		superConnect(api_source + 'travelers/search', traveler_params, function(obj) {
			console.debug("Returned Obj From Assitants Travelers Search") ; 
			console.debug(obj) ; 
			if (obj == null || Object.size(obj) == 0 || obj.error == 1 ||  obj[0].military_num == 0){
				...
				$("#traveler_security_status").html(passenger_security_options) ;
...
//			traveler_position = (obj[0].job_description == 1)?('<option selected value ='+1+'>'+ 'أساسى' + '</option>'):('<option selected value ='+2+'>'+ 'إحتياطى' + '</option>') ;
			traveler_sec_status = (obj[0].security_status == 1)?('<option selected value ='+1+'>'+ 'لا يوجد' + '</option>'):('<option selected value ='+2+'>'+ 'يوجد' + '</option>') ; 	
			$("#traveler_rank").html(traveler_rank) ; 
			$("#traveler_name").val(obj[0].name) ;
			$("#traveler_unit").html(traverler_unit) ;
//		    $("#traveler_position").val("") ; 
	...
	var global_ship_type = ship_type[GlobalShipEntryID] ; 
	var global_port_name = port_name[GlobalPortID] ; 
	...
	$(document).ready(function() {
		...
		for ( var i = 0; i < size; i++)
			if (rank[i].rank_type == 2)
				passengers_ranks += '<option value="' + rank[i].id
						+ '">' + rank[i].rank_name + '</option>';
		
		if (ma2morya_type == 1){
			// $('#Ma2morya_name').html("كشف بأسماء ضباط الصف المقرر سفرهم إلى " + " " + "(" + nashat_type +  ")" + " " +  nashat_name + " على متن " + global_ship_name);
